 ...
 * @since  1.0.0
 */
class ProfolioBuilder_Profolio_map extends Widget_Base {
		...
		foreach ( $result_params as $key => $param ) {
			...
			if ( 'Infowindow' === $key ) {
				$this->add_group_control(
					...
					[
						'name'     => 'infowindow_title_typography',
						'label'    => esc_html__( 'Title typography','localseomap-for-elementor'),
						'selector' => '{{WRAPPER}} .profolio-header-xxs',
				...
				$this->add_group_control(
					Group_Control_Typography::get_type(),
					[
						'name'     => 'infowindow_content_typography',
						'label'    => esc_html__( 'Content typography','localseomap-for-elementor'),
						'selector' => '{{WRAPPER}} .profolio-text-xs',
