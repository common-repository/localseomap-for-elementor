...
class Converter {
	...
	public function convert_params( $all_params = array(), $type_from = '', $replace_to = '' ) {
		...
		$file = PROFOLIO_BUILDER_PATH . $replace_to . '/compatibility.php';
	...
	public function get_name( $params = null, $type_from = null ) {
		...
		if ( $type_from == 'wpbakery' && ! empty( $params['name'] ) ) {
			return sanitize_title( $params['name'] );
		}

		return '';
	}

	public function get_title( $params = null, $type_from = null ) {
	...
	public function get_category( $params = null, $type_from = null ) {

		if ( $type_from == 'wpbakery' && ! empty( $params['category'] ) ) {
	...
	public function get_slug( $params = null, $type_from = null ) {
		...
		if ( $type_from == 'wpbakery' ) {
			$suf = '';
			if ( ! empty( $params['suf'] ) ) {
				$suf = $params['suf'];
			}

			return sanitize_title( $params['base'] . $suf );
		}


		return sanitize_title( $params['base'] );
	}
	 ...
	 * @access public
	 */
	public function get_icon( $params = null, $type_from = null, $type_to = null ) {

		if ( $type_to == 'elementor' ) {
			return '';
		}
